...
describe("Router", function () {
  it("可以在 Application.use 中直接使用", function (done) {
    ...
    app.use("/", router);
    request(app.server).post("/ok").expect(200).expect("yes", done);
  });
  ...
  it("可拦截出错信息，并响应200", function (done) {
    ...
    app.use("/", router);
    request(app.server).get("/xx").expect(200).expect("ok", done);
  });
  ...
  it("use() 的中间件始终在 get()、post() 等方法前面", async function () {
    ...
    await request(app.server).post("/ok").expect(200).expect("yes");
    expect(numbers).to.deep.equal([123, 456]);

    await request(app.server).post("/not_ok").expect(200).expect("no");
    expect(numbers).to.deep.equal([123, 456, 123, 456]);

    await request(app.server).get("/").expect(200).expect("home");
  ...
  it("ctx.route 获得原始路由信息", async function () {
    ...
    await request(app.server).post("/ok").expect(200).expect({ method: "POST", path: "/ok" });
    await request(app.server).get("/say/hello").expect(200).expect({ method: "GET", path: "/say/:msg" });
    await request(app.server).get("/say/hi").expect(200).expect({ method: "GET", path: "/say/:msg" });
    ...
    await request(app.server).get("/error/xxxx").expect(200).expect({ method: "GET", path: "/error/:msg" });
